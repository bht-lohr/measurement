...
function take_measurement_Wafeform(FlagSnapShot) {
	//console.log(FlagSnapShot);
	if (FlagSnapShot=="yesSnapShot") snapshot();
    measurementWave ();
} 

function measurementWave() {

	canvas2.width = (iSYL ) + canvas2_distance + canvas2_text  ;
	...
	MWColumn = parseFloat (document.getElementById("MWcolumn").value);
		colorimetry2(colorimetryVal,iSYP,iSYL); 
}
 function colorimetry2(vals,iSYP,iSYL) {
var val1, val2, val3;
	colorimetryVal =vals;
	...
	if (colorimetryVal ==0) {
	  	colorbars2(c2_ctx, 200/100, snapShot_bufferL, iSYP, iSYL, colorGreen2,  (canvas1_hight/2-canvas2_distance*2)*1);
		colorbars2(c2_ctx, 200/100, snapShot_bufferR, iSYP, iSYL, colorBlue,  (canvas1_hight/2-canvas2_distance*2)*1);
		colorbars2(c2_ctx, 200/100, snapShot_bufferM, iSYP, iSYL, colorWhite,  (canvas1_hight/2-canvas2_distance*2)*1);
...
function gridWaveform(iCan) {   
	...
	draw_gridHeader(iCan,canvas1_hight*0 + canvas2_distance*0,0,unitVal);
	draw_gridLine(iCan,canvas1_hight*0 + canvas2_distance*1,1,unitVal);
  	draw_gridLine(iCan,canvas1_hight*0 + canvas2_distance*1 +((canvas1_hight-195)/(canvas2_hight/canvas2_hight)),2,unitVal);
	draw_gridLine(iCan,canvas1_hight*0 + canvas2_distance*1 +((canvas1_hight-150)/(canvas2_hight/canvas2_hight)),7,unitVal);
	draw_gridLine(iCan,canvas1_hight*0 + canvas2_distance*1 +((canvas1_hight-105)/(canvas2_hight/canvas2_hight)),12,unitVal);
	draw_gridLine(iCan,canvas1_hight*1 + canvas2_distance*1,13,unitVal);
...
function StoreMeasurmentWeb_GL() {
...
//  console.log(event);
 ...
 //    channelOutL = event.outputBuffer.getChannelData(0); // no output
 //    channelOutR = event.outputBuffer.getChannelData(1);
      ...
      for(i = 0; i < event.inputBuffer.length; i++) {
        ...
        Loudness_bufferR[i] = channelIn_r[i];
 		   iAmplitude =(Loudness_bufferL[i]+Loudness_bufferR[i])/2;
...
function  dbLU(wert) {	 
	 return - 0.691 +(10 * Math.log(wert+0.000001) / Math.log(10)); // -6dbFS
}
