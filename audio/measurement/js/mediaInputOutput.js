...
var jsNode;
//-------------- var worklet
var isFirtsClick = true;
var audioTracks;
var audioStream = null; //?? videoStream
  var audio ;  
  var BufferSwitcher;
...
function getMousePosAudio(ic_img,iCanvas ,event) { 
  ...
  if (ic_img=="canvas2"){
		 ...
		 if (MWColumn<0)MWColumn=0;
		 if (MWColumn>(iSYL * Wave_Count)-1) MWColumn=(iSYL * Wave_Count)-1;
	     document.getElementById("MWcolumn").value  = MWColumn ;
...
//-----worker start   
	...
	//			_volumeL = event.data.volumeM[0];  
				_input = event.data.input;  
				_output = event.data.output;  
			//	console.log(_input);
				processingAudioMeasurement(_output);
         //   ledsM(_volumeL ,  _volumeR )    
	  BufferSwitcher = jsNode.parameters.get("BufferSwitch");
//	  BufferSwitcher.setValueAtTime(0.0, audioContext.currentTime);
...
function startMedia() {
 ...
 //    video: {deviceId: videoSource ? {exact: videoSource} : undefined}
  ...
  navigator.mediaDevices.getUserMedia(constraints).then(gotStreamMedia).then(gotDevices).catch(handleError);
...
function gotStreamMedia(stream) {
		   console.log ("Access to Audio");
	//
/*	initAudioPlayer();
    jsNode.onaudioprocess = processingAudioMeasurement;   
	realAudioInput = audioContext.createMediaStreamSource(stream);
